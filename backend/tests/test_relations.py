...
class TestRelationsRouter:
    ...
    async def test_get_relations_for_object_with_data(self, test_client):
        ...
        obj1 = await test_client.post("/api/objects", json={
            "title": "Object 1", "content": "Content 1", "type": "note",
        })
        obj2 = await test_client.post("/api/objects", json={
        ...
        create_resp = await test_client.post("/api/relations", json=relation_data)
        assert create_resp.status_code == 200

        # Get relations for source object
        response = await test_client.get(f"/api/relations/object/{obj1.json()['id']}")
        assert response.status_code == 200
        data = response.json()
        assert "relations" in data
    ...
    async def test_create_relation_invalid_type(self, test_client):
        ...
        response = await test_client.post("/api/relations", json={
            "source_id": obj1.json()["id"],
            "source_type": "object",
            "target_id": obj2.json()["id"],
            # missing target_type
    ...
    async def test_delete_relation_success(self, test_client):
        ...
        response = await test_client.delete(f"/api/relations/{relation_id}")
        assert response.status_code == 200
    ...
    async def test_update_relation_success(self, test_client):
        ...
        response = await test_client.put(f"/api/relations/{relation_id}", json=update_data)
        assert response.status_code == 200
