...
class TestSearchRouter:
    ...
    async def test_search_empty_query(self, test_client):
        ...
        response = await test_client.get("/api/search", params={"q": ""})
    ...
    async def test_search_with_type_filter(self, test_client):
        ...
        response = await test_client.get("/api/search", params={
    ...
    async def test_search_with_limit(self, test_client):
        ...
        response = await test_client.get("/api/search", params={"q": "test", "limit": 5})
        assert response.status_code == 200
    ...
    async def test_search_exact_mode(self, test_client):
        ...
        response = await test_client.get("/api/search", params={"q": "python", "exact": True})
        assert response.status_code == 200
        data = response.json()
        assert "results" in data
    ...
    async def test_search_no_results(self, test_client):
        ...
        response = await test_client.get("/api/search", params={"q": "zzznonexistentxyz123"})
        assert response.status_code == 200
        data = response.json()
        assert data["results"] == []
    ...
    async def test_find_similar_success(self, test_client):
        ...
        response = await test_client.get(f"/api/search/similar/{obj_id}")
        assert response.status_code == 200
        data = response.json()
        assert "results" in data
    ...
    async def test_find_similar_not_found(self, test_client):
        ...
        response = await test_client.get("/api/search/similar/non-existent")
        assert response.status_code == 404

    async def test_search_special_characters(self, test_client):
        """Test search with special characters."""
        response = await test_client.get("/api/search", params={"q": "test & <special>"})
        assert response.status_code == 200

    async def test_search_unicode(self, test_client):
        """Test search with unicode characters."""
        response = await test_client.get("/api/search", params={"q": "日本語テスト"})
        assert response.status_code == 200
